	...
	private void initBitmap(Canvas canvas) {
		if (sourceBitmap != null) {
			...
			if (bitmapWidth > width || bitmapHeight > height) {
				...
				// 当图片高度大于屏幕高度时，将图片等比例压缩，使它可以完全显示出来
				float ratio1 = height / (bitmapHeight * 1.0f);// 屏幕高除以图片高(数值越大说明图片所占比例越小)
				float ratio2 = width / (bitmapWidth * 1.0f);// 屏幕宽除以图片宽(数值越大说明图片所占比例越小)
				float ratio = 0;
				float translateX = 0;
				float translateY = 0;
				if (ratio1 > ratio2) {// 图片高所占比大于宽所占比,按宽等比例放大
					ratio = ratio2;
					translateY = (height - sourceBitmap.getHeight() * ratio) / 2f;
				} else {// 图片高所占比小于宽所占比,按高等比例放大
					ratio = ratio1;
