...
import com.pact.healthapp.view.imagepick.adapter.PhotoWallAdapter;
...
public class PhotoWallActivity extends BaseFragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mFragmentManager.beginTransaction()
				.replace(R.id.login_fl_continer, new PhotoWallFragment())
				.addToBackStack(" PhotoWallFragment").commit();
	}

	class PhotoWallFragment extends BaseFragment {
		private View view;
		...
		public View onCreateView(LayoutInflater inflater,
			...
			view = inflater.inflate(R.layout.view_imagepick_photo_wall, null);
			ViewUtils.inject(this, view);
			mPhotoWall = (GridView) view
					.findViewById(R.id.view_imagepick_wall_gv);
	...
	private void updateView(int code, String folderPath) {
		...
		if (code == 100) { // 某个相册
			...
			home_titile1.setText(folderName);
			list.addAll(getAllImagePathsByFolder(folderPath));
		} else if (code == 200) { // 最近照片
			home_titile1.setText(R.string.latest_image);
