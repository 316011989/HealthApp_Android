...
public class AddItemChoicedFragment extends BaseFragment implements View.OnClickListener {
    private SharedPreferenceUtils sp = new SharedPreferenceUtils();
    private View view;
    @ViewInject(R.id.additems_choiced_lv)
    private ExpandableListView eListView;//已选体检项包
    @ViewInject(R.id.additems_next)
    private TextView add_items_next;//下一步
    @ViewInject(R.id.additems_clear_choiced)
    private ImageView add_items_showchoose;//清空

    //之前选中过的以散项形式保存的体检项构造成包
    private List<AddItemBean> list = new ArrayList<AddItemBean>();


    private AddItemChoicedAdapter adapter;
    ...
    public View onCreateView(LayoutInflater inflater,
        ...
        view = inflater.inflate(R.layout.appointment_additem_choiced_layout, null);
        ViewUtils.inject(this, view);
        getActivity().setTitle("已选体检项");
        add_items_next.setOnClickListener(this);
        add_items_showchoose.setOnClickListener(this);
        addItemsList();//改造已选体检项成包状
        positiveSequence();//排序
        adapter = new AddItemChoicedAdapter(context, list);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.additems_next://下一步
                ...
                break;
            case R.id.additems_clear_choiced:
                list.clear();
                ServiceOrderData.checkedChildren = new ArrayList<AddItemBean.PackageDatasEntity>();
                ServiceOrderData.checkedParents = new ArrayList<AddItemBean>();
...
//                        super.onSuccess(arg0);
    ...
    private void addItemsList() {
        List<AddItemBean.PackageDatasEntity> packagelist = new ArrayList<AddItemBean.PackageDatasEntity>();
        packagelist.addAll(ServiceOrderData.checkedChildren);
        list.clear();
