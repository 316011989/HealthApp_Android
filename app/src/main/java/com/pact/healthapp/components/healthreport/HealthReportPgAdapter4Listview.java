 ...
 */
public class HealthReportPgAdapter4Listview extends BaseAdapter {
	private Context context;
	private List<MyReportsPgBean> list;
	private HealthCallBack callBack;

	public HealthReportPgAdapter4Listview(Context context,
			List<MyReportsPgBean> list, HealthCallBack callBack) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		holder.img.setBackgroundResource(R.mipmap.health_report_default);
		...
		String fileName = list
				.get(position)
				.getFileUrl()
				.substring(list.get(position).getFileUrl().length() - 19,
						list.get(position).getFileUrl().length() - 4);
		File file = new File(Environment.getExternalStorageDirectory()
				+ "/ncihealth/" + fileName + ".zip");
		...
		if (holder.download.getText().equals("重新下载")) {
			holder.download.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					// TODO Auto-generated method stub
					callBack.callBack(position, 1, true);
				}
